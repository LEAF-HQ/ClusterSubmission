...
class TimeFormat:
    ...
    def __lt__(self, other):
        if self.time["d"] < other.time["d"]:
            return True
        elif self.time["d"] > other.time["d"]:
            return False
        elif self.time["h"] < other.time["h"]:
            return True
        elif self.time["h"] > other.time["h"]:
            return False
        elif self.time["m"] < other.time["m"]:
            return True
        elif self.time["m"] > other.time["m"]:
            return False
        elif self.time["s"] < other.time["s"]:
            return True
        else:
            return False
...
class ClusterSpecificSettings:
    ...
    def setJobTimeUpperLimit(self, ref_time="01:00:00"):
        ...
        if ref_time in timedict:
            self.Settings["MaxRunTime"] = (
                cluster_time_name,
                '"' + ref_time + '"' if "+JobFlavour" in cluster_time_name else str(timedict[ref_time]),
            )
            return
        ...
        for time_ in timedict:
            if ref_time_ < timedict[time_]:
                self.Settings["MaxRunTime"] = (
                    self.Settings["MaxRunTime"][0],
                    str('"' + time_ + '"' if "+JobFlavour" in cluster_time_name else time_),
                )
                return
